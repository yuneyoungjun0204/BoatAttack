...
namespace BoatAttack
{
    ...
    /// - 에피소드 시작/종료
    /// - 위치 리셋
    /// ⚠️ DefenseEnvController와 함께 사용 (보상 분배는 DefenseEnvController에서 처리)
    /// </summary>
    public class DefenseBoatManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (webObject != null)
            {
                ...
                // DefenseEnvController 참조 전달
                if (envController != null)
                {
                    _webDetector.envController = envController;
                }
            }
            ...
            if (defenseAgent1 != null && defenseAgent2 != null)
            {
                ...
                // 적군 배열 설정 (DefenseEnvController에서 설정된 적군 사용)
                if (envController != null && envController.enemyShips != null)
                {
                    defenseAgent1.enemyShips = envController.enemyShips;
                    defenseAgent2.enemyShips = envController.enemyShips;
                }
                else if (attackBoat != null)
                {
                    // Fallback: 단일 적군을 배열로 변환
                    defenseAgent1.enemyShips = new GameObject[] { attackBoat };
                    defenseAgent2.enemyShips = new GameObject[] { attackBoat };
                }
            }
        ...
        private void Update()
        {
            ...
            if (elapsedTime >= maxEpisodeTime)
            {
                ...
                // 타임아웃 페널티 (그룹 보상으로 처리)
                if (envController != null && envController.m_AgentGroup != null)
                {
                    envController.m_AgentGroup.AddGroupReward(timeoutPenalty);
                    envController.m_AgentGroup.EndGroupEpisode();
                }
                else
                {
                    // Fallback: 개별 보상 (호환성)
