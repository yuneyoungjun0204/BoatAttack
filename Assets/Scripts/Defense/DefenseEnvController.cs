...
namespace BoatAttack
{
    ...
    public class DefenseEnvController : MonoBehaviour
    {
        ...
        public void ResetScene()
        {
            ...
            // 공격선의 pending Invoke/코루틴 취소 (AttackBoatDisabler.DisableBoat 등)
            // Invoke는 StopAllCoroutines로 취소되지 않으므로 각 공격선에서 직접 취소
        ...
        /// <summary>
        /// 공격선의 모든 pending Invoke와 코루틴을 취소
        /// AttackBoatDisabler.DisableBoat 등 에피소드 리셋 후에 실행되는 것을 방지
        /// </summary>
        private void CancelAttackBoatPendingActions()
        {
            foreach (var boat in _attackBoats)
            {
                ...
                // AttackBoatDisabler의 pending Invoke 취소
                var disabler = boat.GetComponent<AttackBoatDisabler>();
                if (disabler != null)
                {
                    disabler.CancelInvoke();
                    // _hasExploded 상태도 리셋 (OnEnable에서도 처리하지만 안전장치)
                }

                // AttackAgent의 pending 코루틴 취소
        ...
        private System.Collections.IEnumerator DestroyAttackBoatNextFrame(GameObject attackBoat)
        {
            ...
            // 먼저 비활성화 → ResetAttackBoatsToOrigin에서 씬 검색 시 제외되어 즉시 재생성됨
            // (Destroy는 프레임 끝에 처리되므로, 비활성화하지 않으면 다음 에피소드에서 재생성 누락)
        ...
        private void ResetAttackBoatsToOrigin()
        {
            ...
            // 비활성화된 공격선 재활성화 (AttackBoatDisabler.DisableBoat에서 SetActive(false)된 경우)
            foreach (var boat in _attackBoats)
            {
