    ...
    public class ObiClothEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (actor.clothBlueprint == null)
            {
                ...
                {
                    string path = EditorUtility.SaveFilePanel("Save blueprint", "Assets/", "ClothBlueprint", "asset");
                    if (!string.IsNullOrEmpty(path))
                    {
                        path = FileUtil.GetProjectRelativePath(path);
                        ObiClothBlueprint asset = ScriptableObject.CreateInstance<ObiClothBlueprint>();
