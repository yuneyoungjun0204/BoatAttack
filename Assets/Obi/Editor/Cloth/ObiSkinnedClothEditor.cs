 ...
 */
    ...
    public class ObiSkinnedClothEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (actor.skinnedClothBlueprint == null)
            {
                if (GUILayout.Button("Create", EditorStyles.miniButton, GUILayout.MaxWidth(80)))
                {
                    string path = EditorUtility.SaveFilePanel("Save blueprint", "Assets/", "SkinnedClothBlueprint", "asset");
                    if (!string.IsNullOrEmpty(path))
                    {
                        path = FileUtil.GetProjectRelativePath(path);
                        ObiSkinnedClothBlueprint asset = ScriptableObject.CreateInstance<ObiSkinnedClothBlueprint>();

                        AssetDatabase.CreateAsset(asset, path);
                        AssetDatabase.SaveAssets();

                        actor.skinnedClothBlueprint = asset;
                    }
                }
            }
