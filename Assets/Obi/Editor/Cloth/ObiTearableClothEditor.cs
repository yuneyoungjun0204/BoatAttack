 ...
 */
    ...
    public class ObiTearableClothEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (actor.clothBlueprint == null)
            {
                if (GUILayout.Button("Create", EditorStyles.miniButton, GUILayout.MaxWidth(80)))
                {
                    string path = EditorUtility.SaveFilePanel("Save blueprint", "Assets/", "TearableClothBlueprint", "asset");
                    if (!string.IsNullOrEmpty(path))
                    {
                        path = FileUtil.GetProjectRelativePath(path);
                        ObiTearableClothBlueprint asset = ScriptableObject.CreateInstance<ObiTearableClothBlueprint>();

                        AssetDatabase.CreateAsset(asset, path);
                        AssetDatabase.SaveAssets();

                        actor.clothBlueprint = asset;
                    }
                }
            }
