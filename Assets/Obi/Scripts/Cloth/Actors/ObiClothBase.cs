...
namespace Obi
{
    public abstract class ObiClothBase : ObiActor, IDistanceConstraintsUser, IBendConstraintsUser, IAerodynamicConstraintsUser
    {
        ...
        [HideInInspector] [NonSerialized] protected int triangleSetID = 0;   /**< Offset of deformable triangles in curent solver*/
        ...
        public override void ProvideDeformableTriangles(ObiNativeIntList deformableTriangles, ObiNativeVector2List deformableUVs)
        {
            if (clothBlueprintBase != null && clothBlueprintBase.deformableTriangles != null)
            {
                ...
                for (int i = 0; i < clothBlueprintBase.deformableTriangles.Length; ++i)
                    deformableTriangles.Add(solverIndices[clothBlueprintBase.deformableTriangles[i]]);

                deformableUVs.AddRange(clothBlueprintBase.triangleUVs);
            }
        }
    }
}
